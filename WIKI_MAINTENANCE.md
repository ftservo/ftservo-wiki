...
- `sdk/`：官方 SDK 子模块。
- `ENGINEERING_RESOURCES.md`：资料要求与型号页模板。

新增公开页面时必须同时添加中英文文件和导航。文件名使用小写英文与连字符。
...
4. 12 V 产品的输入电压统一发布为 `9–12.6 V`；24 V 产品统一发布为 `18–25.2 V`。
...
先保存原始 CSV、测试方法和测试设备信息，再导出清晰的 SVG 或 PNG。图中注明输入电压、环境温度、测量次数、负载方式、单位和连续运行条件。堵转扭矩不能表述为持续工作扭矩。
