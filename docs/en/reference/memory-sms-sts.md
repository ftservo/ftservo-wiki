# SMS / STS Memory Table

For STS half-duplex TTL and SMS RS485 servos. A 16-bit value is serialized low byte first.

The addresses below match the `SMS_STS` application layer in the official FEETECH SDK. Read and write two-byte values with this layer's byte order.
