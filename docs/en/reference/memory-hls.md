...
For HLS half-duplex TTL servos. A 16-bit value is serialized low byte first. HLS addresses 44–45 are goal torque, unlike goal time in SMS/STS.

The addresses below match the `HLSCL` application layer in the official FEETECH SDK. Read and write two-byte values with this layer's byte order.

## Read-only persistent area

| Address | Length | Name | Access |
| ---: | ---: | --- | --- |
| 3–4 | 2 bytes | Model identifier | Read only |
...
| 7 | 1 byte | Secondary ID | Read/write |
...
| 31–32 | 2 bytes | Position offset | Read/write |
| 33 | 1 byte | Operating mode | Read/write |
...
| 41 | 1 byte | Acceleration | Read/write |
| 42–43 | 2 bytes | Goal position | Read/write |
| 44–45 | 2 bytes | Goal torque | Read/write |
| 46–47 | 2 bytes | Goal speed | Read/write |
| 48–49 | 2 bytes | Torque limit | Read/write |
| 55 | 1 byte | Lock flag | Read/write |
